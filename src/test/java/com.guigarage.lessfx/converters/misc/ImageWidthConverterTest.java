...
public class ImageWidthConverterTest extends MiscTest {
    ImageWidthConverter converter;

    @Before
    public void setUp() throws Exception {
        converter = ImageWidthConverter.getInstance();
    }

    @Override
    public void testColor() {
        String input = "image-width(#FFFFFF)";
    ...
    public void testString() {
        String input = "image-width(\"NonexistingFile.png\")";
        ParsedValue<String, Size> value = new ParsedValueImpl<>(input, converter);
        Size result = converter.convert(value, null);
    ...
    public void testUnit() {
        String input = "image-width(1.2px)";
        ParsedValue<String, Size> value = new ParsedValueImpl<>(input, converter);
        Size result = converter.convert(value, null);
    ...
    public void testImage() {
        ...
        for (String image : images) {
            String input = "image-width(\"TestImages/" + image + "\")";
            ParsedValue<String, Size> value = new ParsedValueImpl<>(input, converter);
            Size result = converter.convert(value, null);

            assertNotNull(image + " Not Null", result);
            assertEquals(image + " Width", result.getValue(), 20.0, 0.1);
        }
    }
}